...
// Global variables to store spirits' positions and direction
int enemy10Y = 0;
int enemy20Y = 0;
int enemy30Y = 0;
int bonusY = 0;
int direction = 1; // 1 for down, -1 for up
int cycles = 0;    // Number of cycles completed
...
int main(void) {
  ...
  Nokia5110_PrintBMP(33, 47 - PLAYERH, Bunker0, 0);
	Timer2_Init(80000); // 10ms delay at 80 MHz
  while(1) {
  }
}
...
void Timer2A_Handler(void) {
  ...
  // Update the y-coordinate of each spirit based on the direction
  enemy10Y += direction;
  enemy20Y += direction;
  enemy30Y += direction;
  bonusY += direction;
  ...
  Nokia5110_ClearBuffer();
  Nokia5110_PrintBMP(0, enemy10Y, SmallEnemy10PointA, 0);
  Nokia5110_PrintBMP(16, enemy20Y, SmallEnemy20PointA, 0);
  Nokia5110_PrintBMP(32, enemy30Y, SmallEnemy20PointA, 0);
  Nokia5110_PrintBMP(48, enemy30Y, SmallEnemy30PointA, 0);
  Nokia5110_PrintBMP(64, bonusY, SmallEnemy30PointA, 0);
  Nokia5110_DisplayBuffer();

  // Check if a cycle is completed
  if (enemy10Y >= 47 || enemy10Y <= 0) {
    cycles++;  // Increment the cycle count
    // Reverse the direction
    direction *= -1;
		if(cycles==5){cycles=0;}
  }
  
  // Check if 5 cycles are completed
  if (cycles >= 5) {
    // Reset the cycle count and direction
    cycles = 0;
    direction = 1; // Start moving down again
  }
}
